...
2. **SpecialCharsRule** — Нет знаков `!` `?`.
3. **SensitiveDataRule** — Нет паролей, токенов и т.д.
4. **EnglishOnlyRule** — Только английский язык.

## ⚠️ Ограничения (Known Issues)
Текущая версия анализатора работает в режиме MVP и проверяет **только строковые литералы**.

1. **Только явные строки:** Линтер проверит `slog.Info("Error")` или `slog.Info("Error " + "id")`.
2. **Игнорирование динамики:** Переменные, константы и вызовы функций (например, `fmt.Sprintf`, `logger.Info(err.Error())`) **игнорируются**, чтобы избежать ложных срабатываний.
3. **Позиция аргумента:** Сообщение лога ожидается первым аргументом (стандартное поведение `Info`, `Debug` и т.д.).
